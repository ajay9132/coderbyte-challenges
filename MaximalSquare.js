...
function MaximalSquare(strArr) {
  console.log(`Given array of matrices ${strArr}`);
  for (var i = 0; i < strArr.length; i++) {
    var rowMatrix = strArr[i].split('');
    console.log(`Traversing throug the ${i+1}st row of the matrix with the values ${rowMatrix}`);
    for (var j = 0; i < rowMatrix.length; j++) {
      if (rowMatrix[j] === 0) {
